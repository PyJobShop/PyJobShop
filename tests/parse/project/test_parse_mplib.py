...
def test_mplib_set1():
    ...
    assert_equal(len(instance.projects), 6)
    for project in instance.projects:
        assert_equal(len(project.activities), 62)
        assert_equal(project.release_date, 0)

    assert_equal(len(instance.activities), 6 * 62)
...
def test_mplib_set2():
    ...
    assert_equal(len(instance.resources), 5)
    assert_equal(capacities, [48, 48, 46, 50, 48])
    assert_equal(renewables, [True, True, True, True, True])

    assert_equal(len(instance.projects), 10)
    for project in instance.projects:
        assert_equal(len(project.activities), 52)
        assert_equal(project.release_date, 0)

    assert_equal(len(instance.activities), 10 * 52)
