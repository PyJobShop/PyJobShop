...
def test_instance_single_mode():
    ...
    assert_equal(len(instance.projects[0].activities), 18)
    assert_equal(len(instance.activities), 18)

    activity = instance.activities[1]  # second activity (jobnr. 2)
    successors = [4, 8]
    assert_equal(activity.successors, successors)

    assert_equal(len(activity.modes), 1)
...
def test_instance_mmlib():
    ...
    assert_equal(len(instance.projects), 1)
    assert_equal(len(instance.projects[0].activities), 52)

    assert_equal(len(instance.activities), 52)

    activity = instance.activities[1]  # second activity (jobnr. 2)
    successors = [50, 49, 47, 24, 22, 20, 19, 17, 16, 13]
    assert_equal(activity.successors, successors)

    assert_equal(len(activity.modes), 3)
    assert_equal(activity.modes[0].duration, 2)
