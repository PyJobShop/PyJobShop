...
def test_solve_num_workers(small, solver_, msg, capfd):
    ...
    """
    solve(small, solver_, num_workers=2, display=True)
    printed = capfd.readouterr().out
...
def test_solve_initial_solution(small, solver_, msg, capfd):
    ...
    solve(small, solver_, display=True, initial_solution=init)
    printed = capfd.readouterr().out
...
def test_solve_additional_params(small, capfd, solver_, param, value):
    ...
    solve(small, solver_, display=False)
    printed = capfd.readouterr().out
    assert_equal(printed, "")
    ...
    solve(small, solver_, display=False, **{param: value})
    printed = capfd.readouterr().out
    assert_(printed != "")
