...
class Variables:
    ...
    def _make_is_tardy_variables(self) -> list[IntVar]:
        ...
        for job, job_var in zip(data.jobs, self._jobs):
            ...
            var = model.new_bool_var(f"is_tardy_{job}")
            model.add(job_var.end > job.due_date).only_enforce_if(var)
            model.add(job_var.end <= job.due_date).only_enforce_if(~var)
            is_tardy.append(var)
    ...
    def _make_flow_time_variables(self) -> list[IntVar]:
        ...
        for job, job_var in zip(data.jobs, self._jobs):
            var = model.new_int_var(0, MAX_VALUE, f"flow_time_{job}")
            model.add_max_equality(var, [0, job_var.end - job.release_date])
            flow_times.append(var)
    ...
    def _make_tardiness_variables(self) -> list[IntVar]:
        ...
        for job, job_var in zip(data.jobs, self._jobs):
            assert job.due_date is not None
            var = model.new_int_var(0, MAX_VALUE, f"tardiness_{job}")
            model.add_max_equality(var, [0, job_var.end - job.due_date])
            tardiness.append(var)
    ...
    def _make_earliness_variables(self) -> list[IntVar]:
        ...
        for job, job_var in zip(data.jobs, self._jobs):
            assert job.due_date is not None
            var = model.new_int_var(0, MAX_VALUE, f"earliness_{job}")
            model.add_max_equality(var, [0, job.due_date - job_var.end])
            earliness.append(var)
    ...
    def _make_max_tardiness_variable(self) -> IntVar:
        ...
        if self._jobs:
            ...
            tardiness = [
                job.weight * job_var
                for job, job_var in zip(data.jobs, self.tardiness)
            ]
            model.add_max_equality(max_tardiness_var, tardiness)
    ...
    def warmstart(self, solution: Solution):
        ...
        for task_idx in range(data.num_jobs):
            ...
            if data.objective.weight_total_flow_time > 0:
                value = job_end - job.release_date
                model.add_hint(self.flow_times[task_idx], value)

            if job.due_date is not None:
                if data.objective.weight_tardy_jobs > 0:
                    value = job_end > job.due_date
                    model.add_hint(self.is_tardy[task_idx], value)

                if data.objective.weight_total_tardiness > 0:
                    value = max(0, job_end - job.due_date)
                    model.add_hint(self.tardiness[task_idx], value)

                if data.objective.weight_total_earliness > 0:
                    value = max(0, job.due_date - job_end)
                    model.add_hint(self.earliness[task_idx], value)

                if data.objective.weight_max_tardiness > 0:
                    value = max(0, job_end - job.due_date)
                    max_tardiness = max(max_tardiness, value)
