...
class ORToolsVariables:
    def __init__(self, m: CpModel, data: ProblemData):
        self.job_vars = job_variables(m, data)
        self.task_vars = task_variables(m, data)
        self.task_alt_vars = task_alternative_variables(m, data)
        self.sequence_vars = sequence_variables(m, data, self.task_alt_vars)
